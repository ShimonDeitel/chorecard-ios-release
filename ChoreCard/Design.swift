...
extension Color {
    static let ccAccent = Color(hex: "#007AFF")          // the single accent
    static let ccCard = Color(uiColor: .secondarySystemBackground)
    static let ccCard2 = Color(uiColor: .tertiarySystemBackground)
    static let ccField = Color(uiColor: .tertiarySystemFill)
    static let ccHair = Color(uiColor: .separator)
}

// MARK: - Flat surfaces (cards / pills / buttons)

extension View {
    func ccCard(cornerRadius: CGFloat = 20) -> some View {
        self.padding(16)
            .background(Color.ccCard, in: RoundedRectangle(cornerRadius: cornerRadius, style: .continuous))
    }

    func ccPill() -> some View {
        self.padding(.horizontal, 14).padding(.vertical, 8)
            .background(Color.ccCard, in: Capsule())
    }
